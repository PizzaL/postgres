                 ...
                 double tuple_fraction)
{
    ...
    elog(LOG, "Entering grouping_planner");

    Query      *parse = root->parse;
    elog(LOG, "parse->sortClause=%d", parse->sortClause?1:0);
...
select_rowmark_type(RangeTblEntry *rte, LockClauseStrength strength)
{
    ...
    else
    {
        ...
        elog(LOG, "unrecognized LockClauseStrength %d", (int) strength);
...
create_grouping_paths(PlannerInfo *root,
                      ...
                      grouping_sets_data *gd)
{
    elog(LOG, "Entering create_grouping_paths");
    ...
    elog(LOG, "can_sort=%d can_hash=%d", can_sort, can_hash);
...
create_ordered_paths(PlannerInfo *root,
                     ...
                     double limit_tuples)
{
    elog(LOG, "Entering create_ordered_paths");
...
add_paths_to_grouping_rel(PlannerInfo *root, RelOptInfo *input_rel,
                          ...
                          double dNumGroups, List *havingQual)
{
    elog(LOG, "Entering add_paths_to_grouping_rel");

    Query      *parse = root->parse;
    Path       *cheapest_path = input_rel->cheapest_total_path;
    ListCell   *lc;

    // disabling sorting. For GPU processor, we always 
    // do hash instead of sort.
    // Currently we haven't fully migrated to GPU yet.  
    // Thus only disable sort when hash is possible. 
    if (can_hash)
    {   
        can_sort = false;
    }
